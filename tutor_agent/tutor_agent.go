...
func (t *TutorAgent) buildGraph() error {
	...
	// Set edges
	g.AddEdge("load_documents", "vectorize_documents")    // 改为先向量化
	g.AddEdge("vectorize_documents", "analyze_documents") // 向量化后再分析
...
func (t *TutorAgent) chunkText(text string, chunkSize int, overlap int) []string {
	...
	for _, para := range paragraphs {
		...
		// 如果当前段落加上已有内容超过 chunkSize，保存当前 chunk
...
请用友好、易懂的语言回复。`, len(state.DocumentChunks))
	...
	state.Messages = []llms.MessageContent{
		llms.TextParts(llms.ChatMessageTypeSystem,
			fmt.Sprintf(`你是一位专业的助教，帮助学习者深入理解文档内容。

文档已被切分为 %d 个块并向量化存储。
当回答问题时，系统会自动检索最相关的文档片段提供给你。

你的任务是：
1. 基于检索到的相关内容回答学习者的问题
2. 提供深入的解释和示例
3. 引导学习者思考和探索
4. 用清晰、友好的语言交流

请基于提供的上下文回答问题。`, len(state.DocumentChunks))),
		llms.TextParts(llms.ChatMessageTypeAI, state.DocumentSummary),
	}

	fmt.Println("\n" + strings.Repeat("=", 60))
	fmt.Printf("✅ 分析完成！文档已准备就绪（%d 个向量块）\n", len(state.DocumentChunks))
	fmt.Println(strings.Repeat("=", 60))
...
// 新增函数：retrieveRelevantChunks - 检索相关文档块
func (t *TutorAgent) retrieveRelevantChunks(ctx context.Context, state TutorState, query string, topK int) ([]DocumentChunk, error) {
	...
	for _, result := range results {
		...
		if !exists {
			fmt.Printf("⚠️  警告：NodeID %d 在映射表中不存在\n", result.ID)
			continue
		}

		if chunkIndex >= 0 && chunkIndex < len(state.DocumentChunks) {
			chunk := state.DocumentChunks[chunkIndex] // ⭐ 使用映射后的索引
			...
			fmt.Printf("⚠️  警告：映射后的索引 %d 超出范围 [0, %d)\n", chunkIndex, len(state.DocumentChunks))
		}
	}
...
func (t *TutorAgent) chatWithRAG(ctx context.Context, state TutorState) (TutorState, error) {
	...
	contextBuilder.WriteString("基于以下相关文档内容回答问题：\n\n")

	for i, chunk := range relevantChunks {
		similarity := chunk.Metadata["similarity"]
		contextBuilder.WriteString(fmt.Sprintf("--- 相关片段 %d (相似度: %s, 来源: %s) ---\n",
	...
	contextBuilder.WriteString("请基于上述相关内容，给出准确、详细的回答。如果上述内容不足以回答问题，请说明并尽可能提供帮助。")
