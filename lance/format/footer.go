...
func (f *Footer) Validate() error {
	...
	if f.NumPages < 0 {
		return fmt.Errorf("invalid page count: %d", f.NumPages)
	}
	if f.NumPages != int32(len(f.PageIndexList.Indices)) {
		return fmt.Errorf("page count mismatch: %d vs index size %d", f.NumPages, len(f.PageIndexList.Indices))
	}
	if f.CreatedAt <= 0 {
		return fmt.Errorf("invalid created timestamp: %d", f.CreatedAt)
	}
	return nil
}
